...
func (s *SchedulerServer) handleScheduleTask(w http.ResponseWriter, r *http.Request){
	...
	//Parseo de fecha
	scheduleTime , err := time.Parse(time.RFC3339 , commandReq.ScheduleAt)
	if err != nil{
	...
	unixTimesTemp := time.Unix(scheduleTime.Unix(), 0)

	//Le pasamos el contexto y el objeto a insertar
	taskId , err := s.insertTaskIntoDB(context.Background(), Task{
		Command: commandReq.command, //
		ScheduleAt: pgtype.Timestamp{Time: unixTimesTemp} //Para permitir manejar datos de tipo fecha en PostgresSQL. Por eso pgtype
	})
...
func (s *SchedulerServer) insertTaskIntoDB(ctx context.Context, task Task)(string,error){
	sqlStatement := "INSER INTO task (command,schedule_at) VALUES ($1 $2) RETURNING id"

	var insertedID string

	err := s.dbPool.QueryRow(sqlStatement, task.Command, task.scheduledAt.Time).Scan(&insertedID)
	if err != nil {
	return "", err
	}
